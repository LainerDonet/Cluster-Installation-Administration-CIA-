...
network --device=eth0 --onboot=yes --bootproto=static --ip=172.17.253.253 --netmask=255.255.0.0 --nodns  --hostname=berilium
...
mkdir /cia                                 
mount -o nolock 172.17.100.100:/usr/cia-7.6.1810 /cia  
curl -s 172.17.100.100/berilium#CIA_POST:BEGIN+++++++++++++++ &>/dev/null 
bash -x /cia/cluster_install/bin/ci_post-install-node berilium 172.17.253.253 255.255.0.0 ac:1f:6b:62:7d:08 /usr/cia-7.6.1810 172.17.100.100 
curl -s 172.17.100.100/berilium#CIA_POST:END+++++++++++++++ &>/dev/null 
%end
...
curl -s 172.17.100.100/berilium#CIA_POST:CIA.SHELL &>/dev/null 
done 
