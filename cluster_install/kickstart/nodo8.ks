...
network --device=eth0 --onboot=yes --bootproto=static --ip=8.8.8.8 --netmask=nodo8 --nodns  --hostname=
...
curl -s /172.17.253.253#CIA_POST:BEGIN+++++++++++++++ &>/dev/null 
bash -x /cia/cluster_install/bin/ci_post-install-node 172.17.253.253 8.8.8.8 nodo8  /usr/cia-7.6.1810  
curl -s /172.17.253.253#CIA_POST:END+++++++++++++++ &>/dev/null 
%end
...
curl -s /172.17.253.253#CIA_POST:CIA.SHELL &>/dev/null 
done 
