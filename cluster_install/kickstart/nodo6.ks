...
network --device=eth0 --onboot=yes --bootproto=static --ip=172.17.1.6 --netmask=255.255.0.0 --nodns  --hostname=nodo6
...
mkdir /cia                                 
mount -o nolock :/usr/cia-7.6.1810 /cia  
curl -s /nodo6#CIA_POST:BEGIN+++++++++++++++ &>/dev/null 
bash -x /cia/cluster_install/bin/ci_post-install-node nodo6 172.17.1.6 255.255.0.0 172.17.253.253 /usr/cia-7.6.1810  
curl -s /nodo6#CIA_POST:END+++++++++++++++ &>/dev/null 
%end
...
curl -s /nodo6#CIA_POST:CIA.SHELL &>/dev/null 
done 
