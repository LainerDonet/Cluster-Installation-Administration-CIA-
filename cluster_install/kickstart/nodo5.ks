...
network --device=eth0 --onboot=yes --bootproto=static --ip=172.17.1.5 --netmask=255.255.0.0 --nodns  --hostname=nodo5
...
curl -s 172.17.253.253/nodo5#CIA_POST:BEGIN+++++++++++++++ &>/dev/null 
bash -x /cia/cluster_install/bin/ci_post-install-node nodo5 172.17.1.5 255.255.0.0 3c:ec:ef:56:14:fe /usr/cia-7.6.1810 172.17.253.253 
curl -s 172.17.253.253/nodo5#CIA_POST:END+++++++++++++++ &>/dev/null 
%end
...
curl -s 172.17.253.253/nodo5#CIA_POST:CIA.SHELL &>/dev/null 
done 
